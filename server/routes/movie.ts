...
router.get("/:movie_id", authorization, async (req: Request, res: Response) => {
    console.log("dashboard: GET route");
    try {
        const { movie_id } = req.params
        // console.log(req.user.id);
        const reviews = await db(
            "SELECT r.user_id, r.review_title, r.content, r.created_time FROM movies as m LEFT JOIN movie_reviews as r ON m.movie_id = r.movie_id WHERE m.user_id=$1 ORDER BY r.created_time asc",
            [movie_id]
        );
